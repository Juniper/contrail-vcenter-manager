...
@pytest.fixture()
def task_collector(vm_register_task_info):
    tc = Mock()
    tc.latestPage = [vm_register_task_info]
    return tc


@pytest.fixture()
def esxi_api_client(property_collector, task_collector):
    with patch('cvm.clients.SmartConnectNoSSL') as si_mock:
        si_mock.return_value.content.propertyCollector = property_collector
        si_mock.return_value.content.taskManager.CreateCollectorForTasks.return_value = task_collector
