...
def test_delete_vmi(vnc_api_client, vnc_lib, vnc_vmi):
    vnc_vmi.get_instance_ip_back_refs.return_value = [{'uuid': 'instance-ip-uuid'}]
    ...
    vnc_lib.virtual_machine_interface_delete.assert_called_once_with(id='vmi-uuid')


def test_vmi_no_back_refs(vnc_api_client, vnc_lib, vnc_vmi):
    vnc_vmi.get_instance_ip_back_refs.return_value = None
    vnc_lib.virtual_machine_interface_read.return_value = vnc_vmi
...
def test_delete_vm(vnc_api_client, vnc_lib, vnc_vm, vnc_vmi):
    vnc_vm.get_virtual_machine_interface_back_refs.return_value = [{'uuid': 'vmi-uuid'}]
