...
class VNCAPIClient(object):
    ...
    def _update_vrouter_uuid(cls, vnc_obj, vrouter_uuid):
        ...
        logger.info(vnc_obj.get_annotations())
    ...
    def get_instance_ip_for_vmi(self, vnc_vmi):
        return next(self.read_instance_ip(uuid) for uuid in [instance_ip['uuid'] for instance_ip
                                                             in self.vnc_lib.instance_ips_list()['instance-ips']]
                    if self.read_instance_ip(uuid).get_virtual_machine_interface_refs()[0]['uuid'] == vnc_vmi.uuid)
        #
        # refs = vnc_vmi.get_instance_ip_back_refs()
        # if len(refs) > 0:
