...
class VRouterAPIClient(object):
    ...
    def add_port(self, vmi_model):
        ...
        try:
            ...
            logger.info('Add port response code: %s', response.status_code)
    ...
    def delete_port(self, vmi_uuid):
        ...
        try:
            ...
            logger.info('Delete port response code: %s', response.status_code)
    ...
    def enable_port(self, vmi_uuid):
        try:
            ...
            logger.info('Enable port response code: %s', response.status_code)
    ...
    def disable_port(self, vmi_uuid):
        try:
            request_url = '{host}:{port}/disable-port/{uuid}'.format(host=self.vrouter_host,
                                                                    ...
                                                                    uuid=vmi_uuid)
            response = self._session.put(request_url)
            logger.info('Disable port response code: %s', response.status_code)
