...
class VmRemovedHandler(AbstractEventHandler):
    ...
    def _validate_event(self, event):
        vm_name = event.vm.name
        vm_model = self._vm_service.get_vm_model_by_name(vm_name)
        if not vm_model:
            logger.error('Virtual Machine %s does not exist in CVM database. Unable to remove', vm_name)
            return False
        return True
...
class PowerStateHandler(AbstractChangeHandler):
    ...
    def _validate_vm(self, vmware_vm):
        vm_model = self._vm_service.get_vm_model_by_name(vmware_vm.name)
        if not vm_model:
            logger.error('Virtual Machine %s does not exist in CVM database. Unable to remove', vmware_vm.name)
            return False
        return True
