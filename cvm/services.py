    ...
    def register_vmis(self):
        for vmi_model in list(self._database.vmis_to_update):
            ...
            if existing_vnc_vmi:
                ...
                vmi_model.vn_model = self._database.get_vn_model_by_uuid(
                    existing_vnc_vmi.get_virtual_network_refs()[0]['uuid'])
                existing_instance_ip = self._vnc_api_client.read_instance_ip(
                    existing_vnc_vmi.get_virtual_network_refs()[0]['uuid'])
                if existing_instance_ip:
                    self._vnc_api_client.update_instance_ip_vrouter_uuid(
                        existing_instance_ip, vmi_model.vm_model.vrouter_uuid)
                    vmi_model.vnc_instance_ip = existing_instance_ip
                else:
                    self._add_instance_ip_to(vmi_model)
                self._database.save(vmi_model)
            else:
